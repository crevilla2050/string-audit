...
def handle_projects(args):
    ...
    if args.projects_command == "list":
        ...
        return projects_list(
            cfg["server"],
            cfg.get("active_project"),
            cfg.get("auth", {}).get("token")
        )

    elif args.projects_command == "deleted":
        cfg = load_config()

        return projects_deleted(cfg["server"], cfg.get("api_prefix"), cfg.get("auth", {}).get("token"))
    elif args.projects_command == "restore":
        cfg = load_config()
        ...
        print("Unknown projects subcommand:", args.projects_command)
        return 1    
