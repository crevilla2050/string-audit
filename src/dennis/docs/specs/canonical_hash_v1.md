...
Two compliant implementations must produce identical hashes
for identical normalized input bytes.

Differences in runtime, architecture, or language must not affect results.
...
plan_hash = sha256(canonical_json_bytes)
...
The canonical hash is computed over the entire normalized JSON plan object.
...
Rules:
* Floats SHOULD be avoided in plan schemas. When floats are used, they must be serialized as JSON numbers
using the minimal decimal representation that round-trips exactly
in IEEE-754 double precision.
* No trailing zeros
* Exponential notation is forbidden and MUST NOT be used
...
Unicode strings must be normalized to NFC before canonical JSON serialization.
...
Canonical JSON serialization must produce identical byte sequences
for identical normalized input objects.

Hashing operates on the raw byte sequence, not on parsed structures.
Input JSON documents may contain arbitrary Unicode.
Normalization must be applied before serialization.
